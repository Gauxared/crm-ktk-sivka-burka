# Необязательный локальный адаптер

Статус: экспериментальный. Текущий основной процесс — [облачная разработка](development-pipeline.md).
Запуск только `python scripts/task.py run ID --experimental-local` для preferred=local.
Исторические настройки ниже не считаются активным профилем. Перед каждым экспериментом
проверяются /models, загруженный файл, фактический контекст и параметр reasoning.

Общий JSON-конверт с содержимым файлов оказался отдельным источником ошибок у GPT-OSS
и Qwen3.5. Текущий адаптер не имеет constrained decoding и не восстанавливает JSON молча.
Проектировать новый протокол worker нужно как отдельный эксперимент с критерием полезности,
а не как условие разработки CRM. Цель будущего протокола — минимальное преобразование
вывода, объективные проверки и отдельный учёт транспортных ошибок и дефектов кода.

Граница: локальная модель не владеет shell, Git или секретами. Возврат файлов остаётся
прежним для воспроизводимости экспериментов. Этот переход не притворяется исправлением
качества JSON и не меняет профиль модели.

## Исторические наблюдения bootstrap — не текущая настройка
