...
DB-003 добавляет план общей прогулки и связи участников с заявками. Частичный уникальный
индекс не даёт одной заявке иметь две активные связи, но позволяет нескольким независимым
заявкам участвовать в одной прогулке. Календарного endpoint и команд изменения статуса ещё нет.

## Публичная подача заявки

`POST /api/v1/public/submission-tokens` и `POST /api/v1/public/inquiries` доступны
только когда composition root передал `PublicSubmissionRuntime`: локальный PostgreSQL
engine, непустой HMAC-ключ и адаптер ограничения частоты. Ключ передаётся runtime-ом,
никогда не записывается в БД, логи или `.env.example`; тесты используют только синтетический
ключ. При отсутствии хотя бы этой runtime boundary маршруты возвращают безопасный `503` и
не выполняют запись.

Политика частоты (ключ ограничения, окна, пороги, хранение и ответственный внешний
механизм) ещё не принята. В коде определён только fail-closed интерфейс адаптера, поэтому
локальный запуск не должен подменять его no-op реализацией и не является готовым публичным
развёртыванием.
